...
def save_points_to_txt(points, filename, sS_min, sS_max, q6_min, q6_max):
    """
    Save distributed point coordinates to a txt file in the specified format.
    """
...
def plot_rectangle_and_points(Xmin, Ymin, Xmax, Ymax, points, output_image):
    """
    Plot rectangle boundaries and distributed points, and save as a PNG image.
    """
    ...
    # Set axis range slightly larger than the rectangle
...
def write_sS_q6_to_txt(identifiers, sS_values, q6_values, filename='poscar_ss_q6.txt'):
    """
    Write calculated sS and Q6 values to a txt file.
    """
    with open(filename, 'w', encoding='utf-8') as f:
...
def read_sS_q6_from_txt(filename='poscar_sS_q6.txt'):
    """
    Read sS and Q6 values from a txt file. Return None if file doesn't exist or format is incorrect.
    """
...
def mutate_structure(structure, mutation_rate: float):
    ...
    shear_angle_deg = random.uniform(-1.0, 1.0)
...
def visualize_population_diff(last_sS_q6_values: List[Tuple[float, float]], 
    ...
    plt.title(f'sS vs Q6 - Generation {generation}')
    plt.xlabel('sS')
    plt.ylabel('Q6')
    plt.legend()
...
def main():
    ...
    sS_min_expanded = sS_min - 0.2 * sS_range
    sS_max_expanded = sS_max + 0.2 * sS_range
    q6_min_expanded = q6_min - 0.2 * q6_range
    q6_max_expanded = q6_max + 0.2 * q6_range

    # Distribute target points on the expanded rectangular area
    points_per_side = 10  # Can be modified as needed
    points = distribute_points_on_sides(sS_min_expanded, q6_min_expanded, sS_max_expanded, q6_max_expanded, points_per_side)
    
    # Save these target point information to txt
    save_points_to_txt(points, 'points.txt', sS_min_expanded, sS_max_expanded, q6_min_expanded, q6_max_expanded)
    # Plot rectangle and point distribution
    plot_rectangle_and_points(sS_min_expanded, q6_min_expanded, sS_max_expanded, q6_max_expanded, points, 'rectangle_points.png')

    # Generate targets based on these points
    targets = []
    target_areas = [
